...
import concurrent.futures
        ...
        """
            ...
            if 'who is rushi' in text:
                if client:
                    try:
                        rushi_prompt = """Create a professional, positive response about Rushi"""
                ...
                return "Rushi is an exceptional developer and the owner of this server :)"
            # Werzzzy response - AI generated
            if 'who is werzzzy' in text:
                if client:
                    try:
                        werzzzy_prompt = """Create a professional, positive response about werzzzy"""
                ...
                return "Werzzzy is someone u wouldnt want to mess with..."
            ...
            if any(pattern in text for pattern in ['macro not shaking', 'shake not working', 'why is my macro not shaking', 'macro shaking issue']):
            ...
            # Best rod questions
            if any(pattern in text for pattern in ['best rod', 'what is the best rod', 'top rod', 'good rod']):
            ...
            if any(pattern in text for pattern in ['bloomfisch', 'bloom fisch', 'when is bloomfisch', 'bloomfisch release', 'bloom fisch coming out']):
            ...
            # Fisch wiki/bestiary
            if any(pattern in text for pattern in ['fisch bestiary', 'fisch wiki', 'more information about fisch', 'fischipedia']):
                return "**Fisch Bestiary/More Information:** https://fischipedia.org/wiki/Fisch_Wiki"

            # AFK spots
            if any(pattern in text for pattern in ['afk spot', 'best afk', 'where to afk', 'afk location']):
            ...
            # Appraiser macro
            if any(pattern in text for pattern in ['appraiser macro', 'appraiser bot', 'macro appraiser']):
                return "**Appraiser Macro:**\nCurrently, there is no Appraiser Macro. There will be one in works in the future."

            # Crab cage macro
            if any(pattern in text for pattern in ['crab cage macro', 'crab macro', 'cage macro']):
        ...
        async def on_message(message):
            ...
            special_triggers = ['who is andrew', 'who is rushi', 'who is werzzzy', 'how to read']
